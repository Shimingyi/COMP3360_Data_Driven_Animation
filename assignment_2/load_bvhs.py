...
def add_floor(location=[0, 0, 0]):
    bpy.ops.mesh.primitive_plane_add(size=1000, location=location)
...
def add_bone(offset, parent_obj, name):
    ...
    bpy.ops.mesh.primitive_cone_add(vertices=5, radius1=0.022 * 4, radius2=0.0132 * 4, depth=length, enter_editmode=False, location=center)
...
def build_reset_skeleton(offsets, parents, names, name_appenidx, joint, parent_obj, all_obj):
    if joint != 0:
        offset = mathutils.Vector(offsets[joint])
        new_bone = add_bone(offset, parent_obj, names[joint] + '_bone_%s' % name_appenidx)
        new_joint = add_joint(parent_obj.location + offset, new_bone, names[joint] + '_end_%s' % name_appenidx)
        all_obj.append(new_bone)
        all_obj.append(new_joint)
    else:
        new_joint = add_joint(mathutils.Vector((0., 0., 0.)), None, names[joint] + '_end_%s' % name_appenidx)
        all_obj.append(new_joint)

    for i in range(len(parents)):
        if parents[i] == joint:
            build_reset_skeleton(offsets, parents, names, name_appenidx, i, new_joint, all_obj)
...
def import_bvh(file_path, name_appendix, yoffset=0):
    bpy.ops.import_anim.bvh(filepath=file_path)
    skeleton = bpy.context.object
    skeleton.name = '%s_%s' % (file_path.split('/')[-1].split('.')[0], name_appendix)
    skeleton.location[0] = yoffset
    skeleton.scale = (0.2, 0.2, 0.2)
    return skeleton

def visualize_bvh(file_path, name_appendix, yoffset=0):
    rotations, positions, offsets, parents, names, frametime = load(filename=file_path)
    rotations.qs, positions, offsets = rotations.qs[..., [0, 3, 1, 2]], positions[..., [2, 0, 1]], offsets[..., [2, 0, 1]]
    scale = np.sqrt(np.sum(np.square(offsets), axis=-1)).max() / 2
    positions /= scale
    offsets /= scale
    positions[:, 0, 0] -= np.mean(positions[:, 0, :], axis=0)[0]
    positions[:, 0, 1] -= np.mean(positions[:, 0, :], axis=0)[1]
    positions[:, 0, [1]] = positions[:, 0, [1]] - yoffset
    all_obj = []
    build_reset_skeleton(offsets, parents, names, name_appendix, 0, None, all_obj)
    all_joints = []
    for j in range(rotations.shape[1]):
        name = names[j]
        for obj in all_obj:
            if obj.name == name + '_end_%s' % name_appendix:
...
if __name__ == '__main__':
    ...
    if "--" not in argv:
        ...
        argvs = argv[argv.index("--") + 1:]

    parser = argparse.ArgumentParser(description='BVH visualizer')
    parser.add_argument('-r', '--refer_path', type=str, default='/Users/rubbly/Library/CloudStorage/OneDrive-Personal/Onedrive-文档/Courses/TA-computer-animation/project/assignment2/data/motion_basket.bvh', help='path of reference bvh file')
    parser.add_argument('-c', '--compare_path', type=str, default='/Users/rubbly/Library/CloudStorage/OneDrive-Personal/Onedrive-文档/Courses/TA-computer-animation/project/assignment2/data/motion_basket.bvh', help='path of compared bvh file')
    ...
    clear_objects()
    floor = add_floor()
    camera = add_camera((37.54, -28.87, 16.34), (1.30473, 0.0109881, 0.896417))
    light = add_light((0, 0, 20))

    if args.overlap:
        if args.render:
            refer_skeleton = visualize_bvh(args.refer_path, 'refer')
            compare_skeleton = visualize_bvh(args.compare_path, 'compare')
        else:
            refer_skeleton = import_bvh(args.refer_path, 'refer')
            compare_skeleton = import_bvh(args.compare_path, 'compare')
    else:
        if args.render:
            refer_skeleton = visualize_bvh(args.refer_path, 'refer', -2)
            compare_skeleton = visualize_bvh(args.compare_path, 'compare', 2)
        else:
            refer_skeleton = import_bvh(args.refer_path, 'refer', -2)
            compare_skeleton = import_bvh(args.compare_path, 'compare', 2)
    add_material_for_character(refer_skeleton, (0.989, 0.129, 0.177, 1)) if args.render else None
    add_material_for_character(compare_skeleton, (0.205, 1, 0.376, 1)) if args.render else None
