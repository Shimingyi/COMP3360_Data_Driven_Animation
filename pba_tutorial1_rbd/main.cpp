...
camera_t camera(glm::vec3(0.0f, 2.0f, 4.0f));
...
rigid_body_t rb_structs[NBODIES];

void init_physical_object(rigid_body_t &rb, const trimesh_t &mesh,
                          const glm::vec3 &initial_position,
                          const glm::mat3 &initial_orientation,
                          const glm::vec3 &initial_linear_moment,
                          const glm::vec3 &initial_angular_momentum) {
  // TODO
  // ----
}
...
int main() {
  ...
  object_mesh.load(PROJECT_ROOT_DIR "/cube.obj");
  ...
  glBindVertexArray(object_vao);
  {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 ...
                 &object_mesh.get_vertices()[0], GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(0);
  ...
  const glm::vec3 initial_position(0.f, 2.f, 0.f);
  const glm::mat3 initial_orientation(1.f);
  const glm::vec3 initial_linear_moment(0.f);
  const glm::vec3 initial_angular_momentum(0.f);

  init_physical_object(rb_structs[0], object_mesh, initial_position,
                       initial_orientation, initial_linear_moment,
...
void draw_scene() {
  ...
  // draw sim mesh objects
  {
    ...
    for (int i = 0; i < NBODIES; i++) {
      const rigid_body_t &rb = rb_structs[i];
#if 0 // TODO: enable this once you have defined your rigid body struct
      const glm::vec3 x = rb....; // rigid body position vector
      const glm::mat3 R = rb....; // rigid body orientation matrix
      const glm::mat4 rotation(R);
      const glm::mat4 translation = glm::translate(glm::mat4(1.0), x);
#else
      const glm::mat4 rotation(1.0);
      const glm::mat4 translation =
          glm::translate(glm::mat4(1.0), glm::vec3(0, 2, 0));
#endif
      ...
      glDrawElements(GL_TRIANGLES, object_mesh.get_index_count(),
                     GL_UNSIGNED_INT, 0);
